...
- **開発言語**: Python 3.12+
- **Web UI**: Streamlit（動画アップロード・結果表示）+ Plotly（インタラクティブチャート）
- **デザインシステム**: モダンSaaS風（Stripe/Linear/Notion/Tableau inspired）
- **動画処理**: ffmpeg（動画分割・長さ取得）
- **解析対象**: 面談動画（30分/回）+ 行動心理学ナレッジベース
- **処理能力**: 動画ネイティブ入力対応（マルチモーダル）
- **出力**: 構造化されたJSON形式の評価レポート + HTMLビジュアルレポート
...
│   ├── feedback.py                   # フィードバック送信（GitHub Issues + メール）
...
│   ├── core-criteria.md              # 行動心理学の評価基準（コア）
│   ├── presets/                      # 評価基準プリセット
│   │   └── ses_interview.md          # SES面談向け評価基準
│   └── reference/                    # 参考資料（人間閲覧用・AIプロンプト非投入）
│       ├── README.md                 # 参考資料層の説明
...
| タブ | 内容 |
|------|------|
| 単一動画解析 | 動画アップロード → AI解析 → 結果表示・HTMLレポートダウンロード |
| バッチ処理 | 最大10件一括解析・CSV/JSON/HTML ZIP出力 |
| ナレッジベース | 評価基準の閲覧・参考資料の管理・アップロード |
| 使い方 | 推奨動画仕様・操作手順・注意事項 |
| 設定 | モデル選択・APIキー・Whisper設定 |
| フィードバック | バグ報告・機能リクエスト → GitHub Issue自動作成 + メール通知 |
...
- ✅ 複数APIキーの自動切り替え
- ✅ 結果のビジュアル表示・JSON出力
- ✅ HTMLレポート出力（SVGレーダーチャート・バーグラフ・印刷対応）
- ✅ 時系列分析（チャンク別の詳細評価）
- ✅ バッチ処理（最大10件一括解析・HTML ZIP出力）
- ✅ 2層ナレッジベース（参考資料 + 評価基準）
- ✅ フィードバック機能（GitHub Issues連携 + M365メール通知）
- ✅ **エンタープライズグレードUI/UX**（2026年3月実装）
  - **Plotlyインタラクティブチャート**: レーダーチャート、時系列グラフ、ヒートマップ、バーチャート
  - **モダンデザインシステム**: Stripe/Linear/Notion風のカードコンポーネント
  - **データビジュアライゼーション**: Tableau/Weights & Biases風の候補者比較機能
  - **20サイトベンチマーク**: AI/ML、HR Tech、SaaSのトップサイトからデザインパターンを抽出
...
### ✅ Milestone 5: レポート出力・ナレッジベース強化 - 完了
- [x] HTMLレポート出力機能
  - [x] スタンドアロンHTMLレポート生成（CSS/SVG埋め込み）
  - [x] SVGレーダーチャート（6カテゴリ）
  - [x] SVG横棒グラフ（スコア色分け）
  - [x] 印刷対応CSS（A4レイアウト最適化）
  - [x] 単一解析・バッチ処理両対応
  - [x] バッチ処理のHTML ZIP一括出力
- [x] 2層ナレッジベース構造
  - [x] 参考資料層（reference/）: 人間閲覧用、AIプロンプト非投入
  - [x] 評価基準層（presets/）: AIプロンプトに投入される評価基準
  - [x] ナレッジベース専用タブで参考資料の閲覧・アップロード
  - [x] 評価基準の内容をexpanderで確認可能
- [x] フィードバック機能
  - [x] GitHub Issue自動作成（カテゴリ・ラベル付き）
  - [x] M365 SMTP経由のメール通知
  - [x] フィードバック専用タブ
- [x] UI改善
  - [x] 6タブ構成（単一解析/バッチ/ナレッジベース/使い方/設定/フィードバック）
  - [x] サイドバー廃止、モデル選択を設定タブに統合
  - [x] 使い方ガイドを独立タブ化

### 次のステップ

### Milestone 6: 精度検証とフィードバック（運用後）
